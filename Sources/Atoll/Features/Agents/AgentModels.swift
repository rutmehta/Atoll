...
/// One decoded JSON line received from the hook script over /tmp/atoll.sock.
...
enum AgentPermissionDecision {
    ...
    static func denyJSON(event: String, message: String = "User denied this action from Atoll.") -> String {
        let output: [String: Any] = [
            "hookSpecificOutput": [
                "hookEventName": event,
                "decision": [
