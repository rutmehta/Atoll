...
```swift
AgentSessionStore.shared.start()      // binds /tmp/atoll.sock, discovers existing
...
are idempotent, preserve all non-Atoll config, and mark entries with the
`atoll` path marker.
...
- Socket: `/tmp/atoll.sock`, chmod 0600, stale-socket probe on bind. If a
  second Atoll instance is running, `socketActive == false` (UI can surface).
...
- Hook script installed to
  `~/Library/Application Support/Atoll/atoll-hook.sh` (0755); registered
