...
final class AgentSessionStore: ObservableObject {
    ...
    func start() {
        ...
        server.onPermissionExpired = { [weak self] replyKey in
            MainActor.assumeIsolated {
                guard let self else { return }
                for (key, var session) in self.sessions
                where session.pendingPermission?.key == replyKey {
                    // The CLI is now showing its own fallback prompt: keep the
                    // attention state but drop the stale card so the UI offers
                    // "jump to terminal" instead of dead approve buttons.
    ...
    func ingest(_ envelope: AgentHookEnvelope, replyKey: String?) {
        ...
        case "processing":
            ...
            session.pendingPermission = nil
            if envelope.event == "PostToolUse" || envelope.event == "PostToolUseFailure",
               let toolUseId = envelope.toolUseId {
                session.finishToolEvent(id: Self.toolEventId(toolUseId, provider: envelope.provider),
                                        success: envelope.event != "PostToolUseFailure")
            }
        ...
        case "running_tool":
            ...
            // Codex ids are wrapped so hook-channel and rollout-channel events
            // dedupe onto the same feed entry.
            let id = envelope.toolUseId.map { Self.toolEventId($0, provider: envelope.provider) }
                ?? UUID().uuidString
    ...
    func respondToPermission(sessionKey: String, allow: Bool, remember: Bool = false,
                             denyMessage: String = "User denied this action from Atoll.") {
    ...
    /// Atoll launched still show up (state is a best guess, no PID).
    ...
    /// Codex ids are wrapped (`codex:<call_id>`) so the hook channel and the
    /// rollout-transcript channel dedupe onto one feed entry.
    nonisolated static func toolEventId(_ toolUseId: String, provider: AgentProvider) -> String {
        provider == .codex ? AgentTranscriptParser.codexEventId(toolUseId) : toolUseId
    }

    /// `~/.claude/projects/<sanitized cwd>/<session_id>.jsonl`.
    /// Claude Code keeps ASCII letters/digits and maps every other character
    /// (including '_' and non-ASCII) to '-', without collapsing runs.
    nonisolated static func claudeFallbackTranscriptPath(cwd: String, sessionId: String) -> String {
        let projectDir = String(cwd.map { c -> Character in
