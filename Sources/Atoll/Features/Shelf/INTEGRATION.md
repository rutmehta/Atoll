...
- `~/Library/Application Support/Atoll/shelf.json` — item list (files stored as security-scoped bookmarks with stale-refresh + path-repair)
- `~/Library/Application Support/Atoll/ShelfStorage/` — copies created from raw dropped data / pasted clipboard images (deleted when the item is removed)
