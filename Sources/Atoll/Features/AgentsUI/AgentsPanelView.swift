        ...
        HStack(spacing: 6) {
            ...
            if !store.socketActive {
                Label("Socket unavailable", systemImage: "exclamationmark.triangle.fill")
                    ...
                    .help("Could not bind /tmp/atoll.sock — is another Atoll running?")
            }
        }
    }
    ...
    private var emptyState: some View {
        VStack(spacing: 10) {
            ...
            if installer.overallState == .installed {
                ...
                Text("Atoll watches Claude Code and Codex through lightweight shell hooks. Install them to see live sessions, approve permissions from the notch, and get notified when an agent needs you.")
                    .font(.system(size: 11))
                    .foregroundStyle(Color(white: 0.6))
                    .multilineTextAlignment(.center)
                    .frame(maxWidth: 420)
