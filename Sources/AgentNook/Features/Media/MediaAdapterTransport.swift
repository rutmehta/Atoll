...
/// `.build/debug` and a `dist/AgentNook.app` with the dylib in
...
final class MediaAdapterTransport {
    ...
    private let commandQueue = DispatchQueue(label: "agentnook.media.adapter.commands")
    ...
    static let runScriptPath: String? = {
        ...
        if let res = Bundle.main.resourceURL {
            directories.append(res) // AgentNook.app/Contents/Resources
        }
    ...
    func start() {
        ...
        guard let script = MediaAdapterTransport.runScriptPath,
            ...
            NSLog("AgentNook/Media: MediaRemote adapter resources not found (script or dylib missing)")
            return
        }
        ...
        do {
            ...
            NSLog("AgentNook/Media: failed to launch adapter loop: \(error)")
            process = nil
            stdinPipe = nil
        }
    }
    ...
    private func handleLine(_ lineData: Data) {
        ...
        if lineData == Data("NIL".utf8) {
            DispatchQueue.main.async { [weak self] in self?.onTrackInfo?(nil) }
            return
        }
        do {
            let info = try JSONDecoder().decode(TrackInfo.self, from: lineData)
            DispatchQueue.main.async { [weak self] in self?.onTrackInfo?(info) }
        } catch {
    ...
    private func sendOneShot(_ command: String) {
        ...
        do {
            ...
            NSLog("AgentNook/Media: one-shot adapter command failed: \(error)")
        }
    }
}
