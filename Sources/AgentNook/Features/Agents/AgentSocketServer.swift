...
/// AF_UNIX SOCK_STREAM server at /tmp/agentnook.sock receiving one JSON envelope
...
final class AgentSocketServer {
    ...
    static let socketPath = "/tmp/agentnook.sock"
    ...
    private let queue = DispatchQueue(label: "com.agentnook.agents.socket", qos: .utility)
    ...
    /// (e.g. a second AgentNook instance) or the socket could not be created.
    @discardableResult
    func start() -> Bool {
        var ok = false
    ...
    private func startLocked() -> Bool {
        ...
        if FileManager.default.fileExists(atPath: AgentSocketServer.socketPath) {
            ...
            if probe >= 0 {
                ...
                close(probe)
                if result == 0 {
                    NSLog("AgentNook: another process is already serving \(AgentSocketServer.socketPath); socket server not started")
                    return false
                }
                if err != ECONNREFUSED && err != ENOENT {
                    NSLog("AgentNook: unexpected errno \(err) probing agent socket; unlinking anyway")
                }
            }
            unlink(AgentSocketServer.socketPath)
        }
        ...
        guard fd >= 0 else {
            NSLog("AgentNook: socket() failed: \(String(cString: strerror(errno)))")
            return false
        }
        ...
        guard bindResult == 0 else {
            NSLog("AgentNook: bind(\(AgentSocketServer.socketPath)) failed: \(String(cString: strerror(errno)))")
            close(fd)
            return false
        }
        ...
        guard listen(fd, 16) == 0 else {
            NSLog("AgentNook: listen failed: \(String(cString: strerror(errno)))")
            close(fd)
            unlink(AgentSocketServer.socketPath)
            return false
        }
        ...
        source.resume()
        acceptSource = source
        NSLog("AgentNook: agent socket listening at \(AgentSocketServer.socketPath)")
        return true
    }
