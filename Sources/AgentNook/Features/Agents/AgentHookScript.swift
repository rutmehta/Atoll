...
/// `~/Library/Application Support/AgentNook/agentnook-hook.sh` (chmod 755).
///
/// One script serves both providers — the installer registers it as
/// `agentnook-hook.sh claude` in ~/.claude/settings.json and
/// `agentnook-hook.sh codex` in ~/.codex/hooks.json.
///
/// Behavior:
/// - exits 0 instantly (and silently) when /tmp/agentnook.sock is absent, so the
///   hooks cost ~nothing while AgentNook is not running;
...
///   recv for up to 290s; whatever AgentNook writes back is printed to stdout
...
enum AgentHookScript {
    ...
    static let marker = "agentnook"

    static let scriptFileName = "agentnook-hook.sh"

    static let text: String = #"""
#!/bin/bash
# agentnook-hook.sh — AgentNook agent-session hook (marker: agentnook)
# Installed and managed by AgentNook.app. Do not edit; reinstalling overwrites.
# Usage: agentnook-hook.sh [claude|codex]
SOCKET="${AGENTNOOK_SOCKET:-/tmp/agentnook.sock}"
[ -S "$SOCKET" ] || exit 0
...
sock_path = sys.argv[2] if len(sys.argv) > 2 else "/tmp/agentnook.sock"
...
def find_agent_process(needle):
    ...
    for _ in range(8):
        ...
        lower = cmd.lower()
        if "agentnook" in lower or lower.startswith("/usr/bin/python3"):
...
def main():
    ...
    if event == "PermissionRequest":
        # Hold the line: AgentNook writes the hook-decision JSON back when the
