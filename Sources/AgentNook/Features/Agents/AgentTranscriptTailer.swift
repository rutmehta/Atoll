...
final class AgentTranscriptParser {
    ...
    private func parseCodexResponseItem(_ payload: [String: Any]) -> [AgentTranscriptEvent] {
        ...
        case "function_call":
            ...
            let isShell = ["exec_command", "shell", "local_shell", "container.exec"].contains(rawName)
    ...
    private static func exitCodeFromText(_ text: String) -> Bool? {
        guard let range = text.range(of: #"exited with code (\d+)"#, options: .regularExpression) else {
            return nil
        }
...
final class AgentTranscriptTailer {
    ...
    private let queue = DispatchQueue(label: "com.agentnook.agents.tailer", qos: .utility)
