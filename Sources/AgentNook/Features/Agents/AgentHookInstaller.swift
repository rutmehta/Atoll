...
/// Opt-in installer for the AgentNook hook script into
/// `~/.claude/settings.json` (respecting `$CLAUDE_CONFIG_DIR`) and
/// `~/.codex/hooks.json`.
///
/// - Idempotent upsert: our entries are recognized by the "agentnook" marker in
...
final class AgentHookInstaller: ObservableObject {
    ...
    /// Where the hook script is written. Contains the marker "agentnook" in the
    /// path, which is what install/uninstall/status detection keys on.
    nonisolated static var scriptURL: URL {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("AgentNook", isDirectory: true)
    ...
    func install() {
        ...
        do {
            ...
            NSLog("AgentNook: hook install failed: \(error.localizedDescription)")
        }
        refreshStatus()
    }
    ...
    func uninstall() {
        ...
        do {
            ...
            lastError = error.localizedDescription
            NSLog("AgentNook: hook uninstall failed: \(error.localizedDescription)")
        }
        refreshStatus()
    }
    ...
    /// All non-AgentNook entries are preserved byte-for-byte (same parsed values).
    ...
    /// Missing or empty file → `[:]`. Present-but-unparseable → throw (never clobber).
