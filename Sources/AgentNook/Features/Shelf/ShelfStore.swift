...
enum ShelfLocations {
    /// ~/Library/Application Support/AgentNook
    static let supportDirectory: URL = {
        let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? URL(fileURLWithPath: NSHomeDirectory()).appendingPathComponent("Library/Application Support", isDirectory: true)
        return base.appendingPathComponent("AgentNook", isDirectory: true)
    }()
...
final class ShelfStore: ObservableObject {
    ...
    private func load() {
        ...
        for var item in decoded {
            if item.isFile {
                // Refresh stale bookmarks; silently drop files that no longer exist.
                if resolveAndRepair(&item) != nil {
                    kept.append(item)
                } else {
                    NSLog("AgentNook shelf: dropping missing file \(item.displayName)")
                }
            } else {
                kept.append(item)
            }
    ...
    private func save() {
        do {
            ...
            NSLog("AgentNook shelf: save failed: \(error)")
        }
    }
    ...
    nonisolated static func writeStorageFile(_ data: Data, baseName: String, fileExtension: String) -> URL? {
        ...
        do {
            ...
            NSLog("AgentNook shelf: failed to store dropped data: \(error)")
            return nil
        }
    }
    ...
    nonisolated static func temporaryTextFile(_ string: String, name: String) -> URL? {
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent("AgentNookQuickLook", isDirectory: true)
        do {
