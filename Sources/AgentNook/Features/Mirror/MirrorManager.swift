...
final class MirrorManager: ObservableObject {
    ...
    private let sessionQueue = DispatchQueue(label: "app.agentnook.mirror.session")
    ...
    private init() {
        ...
        notificationObservers.append(center.addObserver(
            forName: .AVCaptureSessionRuntimeError, object: session, queue: .main) { note in
                ...
                NSLog("AgentNook Mirror: capture session runtime error: %@",
        ...
        sessionQueue.async {
            ...
            do {
                ...
                if session.canAddInput(input) {
                    ...
                    NSLog("AgentNook Mirror: session refused input for %@", device.localizedName)
                }
            } catch {
                NSLog("AgentNook Mirror: failed to create camera input: %@", error.localizedDescription)
            }
            session.commitConfiguration()
            Task { @MainActor in
