...
struct AgentsSettingsView: View {
    ...
    private var hooksSection: some View {
        Section {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("AgentNook registers one shell script in ~/.claude/settings.json and ~/.codex/hooks.json so live sessions, tool activity and permission requests appear in the notch. All existing hook entries are preserved; uninstall removes only AgentNook's entries.")
                if let error = installer.lastError {
                    Text(error).foregroundStyle(.red)
                }
                if !store.socketActive {
                    Text("Socket /tmp/agentnook.sock is not active — hooks will be silent until AgentNook can bind it (is another instance running?).")
    ...
    private var accountsSection: some View {
        Section {
            ...
            Text("Quota data reuses the CLIs' own credentials (Claude Code keychain item, ~/.codex/auth.json). AgentNook never stores or transmits them anywhere else.")
                .font(.callout)
                .foregroundStyle(.secondary)
        }
    }
