...
final class HUDBrightnessManager: ObservableObject {
    ...
    private func loadDisplayServices() {
        ...
        guard let handle = dlopen(path, RTLD_NOW) else {
            NSLog("AgentNook HUD: DisplayServices dlopen failed — brightness HUD disabled")
            isAvailable = false
            return
        }
        ...
        if !isAvailable {
            NSLog("AgentNook HUD: DisplayServices symbols missing — brightness HUD disabled")
        }
    ...
    func refresh() -> Float? {
        ...
        var value: Float = 0
        let status = getBrightnessFn(CGMainDisplayID(), &value)
    ...
    func setBrightness(_ target: Float) {
        ...
        guard setBrightnessFn(CGMainDisplayID(), clamped) == 0 else { return }
        brightness = clamped
        lastChangedAt = Date()
    }
